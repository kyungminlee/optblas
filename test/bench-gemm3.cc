...
extern "C"
void ddgemm_(char const * ptransa, char const * ptransb, int const * pm, int const * pn, int const * pk, real const * palpha,
...
int main() {
    ...
    for(int i=0; i<M*K; ++i) { A[i].hi = 1.0; A[i].lo = 1e-18; }
    #pragma omp parallel for
    for(int i=0; i<K*N; ++i) { B[i].hi = 1.0; B[i].lo = 1e-18; }
    #pragma omp parallel for
    for(int i=0; i<M*N; ++i) { C[i].hi = 0.0; C[i].lo = 0.0; }
